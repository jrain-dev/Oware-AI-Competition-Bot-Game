...
This repository contains a minimal Python implementation of the Oware (Mancala family) board game, a random agent, and a Q-Learning agent. It includes a simple simulation runner, data logger, and comprehensive metrics collection for AI agent decision-making.
...
- Track detailed performance metrics for agent decision-making (timing, nodes expanded, evaluations, etc.)
...
- `simulationRunner.py` — Top-level runner. Instantiates the board, agents, and logger, then runs episodes. Supports optional comprehensive metrics collection via `StatsRecorder`.
- `oware_ai/metrics/` — Metrics collection module for tracking agent decision-making statistics
- `examples/run_with_stats.py` — Example script demonstrating statistics collection
- `tests/test_stats_recorder.py` — Unit tests for metrics collection
...
- `docs/metrics.md` — Comprehensive documentation for the metrics collection system
...
The repository includes comprehensive metrics collection for tracking AI agent performance. See [docs/metrics.md](docs/metrics.md) for full documentation.

Quick start with metrics:

```bash
# Run example with stats collection
python3 examples/run_with_stats.py --matches 3 --csv

# Run simulation with stats via environment variable
STATS_OUTPUT="training_stats.jsonl" python3 -c "from simulationRunner import run_simulation; run_simulation(episodes=100)"
...
- Decision times (avg, median, per-move)
